...
const EditCategoryById = () => {
  ...
  const updateCategory = async (data) => {
    try {
      ...
      // // API call
      await categorySvc.categoryEdit(params.id, data);
 ...
 const loadAllCats = useCallback(async(parentId = null)=>{
  ...
  const getDetail = useCallback(async()=>{
    try{
      const res = await categorySvc.getCategoryDetailById(params.id);
      ...
      toast.error("Error loading category from.");
  ...
  return (
    <>
      <section className="bg-gray-50 p-3 sm:p-5">
        <AdminPageTitle pageTitle={"Category Edit Page"} />
