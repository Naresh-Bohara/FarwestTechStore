...
const CartItemList = ({ item }) => {
  ...
  const updateCart = async(qty=1)=>{
    try{
      const response = await cartSvc.updateCart({
        cartId: item._id,
        quantity: qty
      })
  ...
  const adddToCart = async()=>{
    try{
      const response = await cartSvc.addToCart({
        ...
        <div className="space-y-4 md:flex md:items-center md:justify-between md:gap-6 md:space-y-0">
          ...
          <div className="flex items-center justify-between md:order-3 md:justify-end">
            ...
            <div className="text-end md:order-4 md:w-32">
              <p className="text-base px-2 font-semibold text-gray-900 dark:text-white">
                {
          ...
          <div className="w-full min-w-0 flex-1 space-y-4 md:order-2 md:max-w-md">
            ...
            >
           {
            ...
            <div className="flex items-center gap-4">
              ...
              >
                <FaTimes  className="me-1.5 h-5 w-5" />
                Remove
              </button>
            </div>
          </div>
...
const CartPage = () => {
  ...
  useEffect(()=>{
    ...
    if(cart && cart.length<=0){
      toast.info("Your cart is empty. Please add items to cart before checkout.");
      navigate(`/products`);
    }
  }, [cart]) 
  return (
    <>
      <section className="bg-primary-50 py-8 antialiased dark:bg-gray-900 md:py-16">
        <div className="mx-auto max-w-screen-xl px-4 2xl:px-0">
         ...
         {/* related products ---from order having highest no.of orders product make api and do here to show related products */}
