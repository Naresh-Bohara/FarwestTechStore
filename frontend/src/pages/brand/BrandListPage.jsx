...
const BrandListPage = () => {
  ...
  const loadData = async ({ page = 1, search = null }) => {
    try {
      setLoading(true);
      const res = await brandSvc.getBrandList({ page: page, search: search });
      ...
      toast.error("Error while loading brand data.");
  ...
  const deleteData = async (brandId) => {
    try {
      setLoading(true);
      await brandSvc.deleteBrand(brandId);
  ...
  return (
    <section className="bg-gray-50 dark:bg-gray-900 p-3 sm:p-5">
      <AdminPageTitle pageTitle={"Brand List"} />
