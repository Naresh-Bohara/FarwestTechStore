...
const UserCartViewDropdown = () => {
  ...
  const cartDetail = useSelector(
    (state) => state.cart || { cart: [], counter: 0 }
  );
        ...
        <div className="absolute right-0 z-20 mt-2 w-80 max-h-[400px] overflow-y-auto rounded-lg bg-white p-4 shadow-lg dark:bg-gray-800">
          ...
          {cartDetail.cart && cartDetail.cart.length > 0 ? (
            <>
              ...
              {/* VIEW CART BUTTON */}
              {cartDetail.cart.length > 0 && (
                <NavLink
                  to="/cart"
                  onClick={() => setIsOpen(false)}
                  className="mt-4 inline-flex w-full items-center justify-center rounded-lg bg-primary-700 px-5 py-2.5 text-sm font-medium text-white hover:bg-primary-800"
                >
                  View Cart
                </NavLink>
              )}
